...
enum class API_MESSAGE : uint8_t
{
	...
	API_MESSAGE_ASKED_API_STOP = 0x12,
	API_MESSAGE_ASKED_API_PAUSE = 0x13,
	API_MESSAGE_ASKED_API_RESUME = 0x14,
};
...
inline static int comparePointX ( const void * a , const void * b ){
	if ( ((t_point*)a)->x <  ((t_point*)b)->x ) return -1;
	if ( ((t_point*)a)->x >  ((t_point*)b)->x ) return 1;
	if ( ((t_point*)a)->y <  ((t_point*)b)->y ) return -1;
	if ( ((t_point*)a)->y >  ((t_point*)b)->y ) return 1;
	return 0;
}
